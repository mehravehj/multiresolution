...
def data_loader(dataset, valid_percent, batch_size, indices):
    ...
    if valid_percent:  # create validation set
        ...
        trainloader = torch.utils.data.DataLoader(
            ...
            sampler=torch.utils.data.sampler.SubsetRandomSampler(train_indc), num_workers=4)
        valloader = torch.utils.data.DataLoader(
            trainset, batch_size=batch_size,
            sampler=torch.utils.data.sampler.SubsetRandomSampler(val_indc), num_workers=4)
    ...
    return trainloader, valloader, testloader, indices, num_classgit
